...
UpstreamTagger::UpstreamTagger()
    : FairDetector("UpstreamTagger", kTRUE, kStraw)
    , fTrackID(-1)
    , fVolumeID(-1)
    , fPos()
    , fMom()
...
Bool_t  UpstreamTagger::ProcessHits(FairVolume* vol)
{
  ...
  // Create UpstreamTaggerPoint at exit of active volume
  if ( gMC->IsTrackExiting()    ||
    ...
    // Increment number of UpstreamTagger det points in TParticle
...
std::tuple<Int_t, Int_t, Int_t, Int_t> UpstreamTagger::StrawDecode(Int_t detID)
{
    ...
    if (statnb < 1 || statnb > 4 || vnb < 0 || vnb > 3 || lnb < 0 || lnb > 1 || snb < 1 || snb > 317) {
        LOG(warning) << "Invalid UpstreamTagger detID:";
        LOG(warning) << detID << " -> station: " << statnb << ", view: " << vnb << ", layer: " << lnb
                     << ", straw: " << snb;
        LOG(warning) << "UpstreamTagger detID is 7-digit!";
...
void UpstreamTagger::StrawEndPoints(Int_t fDetectorID, TVector3 &vbot, TVector3 &vtop)
// method to get end points from TGeoNavigator
{
    ...
    if (!rc) {
        LOG(warning) << "UpstreamTagger::StrawDecode, TGeoNavigator failed" << path;
        return;
    }
